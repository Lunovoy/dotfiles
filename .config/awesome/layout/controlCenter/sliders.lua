...
awful.spawn.easy_async_with_shell("brightnessctl -d amdgpu_bl0 | grep -i 'current' | awk '{print $4}' | grep -o '[0-9]\\+'", function (stdout)
...
brightness:connect_signal("property::value", function(_, new_value)
    ...
    awful.spawn("brightnessctl -d amdgpu_bl0 set " .. new_value .."%", false)
end)
...
awful.spawn.easy_async_with_shell("amixer -c1 get Master | tail -n 1 | awk '{print $5}' | grep -o '[0-9]\\+'", function (stdout)
    local value = string.gsub(stdout, '^%s*(.-)%s*$', '%1')
...
volume:connect_signal("property::value", function(_, new_value)
    ...
    awful.spawn("amixer -c1 set Master " .. new_value .."%", false)
end)
