...
awful.screen.connect_for_each_screen(function(s)
	...
	bluetooth:buttons(gears.table.join({
		awful.button({}, 1, function()
			awful.spawn(user_likes.bluetoothmenu, false)
		end),
	}))
	...
	wifi:buttons(gears.table.join({
		awful.button({}, 1, function()
			awful.spawn(user_likes.wifimenu, false)
		end),
	}))
	...
	awesome.connect_signal("signal::wifi", function(value)
		if value then
			wifi.markup = helpers.colorize_text("", "#FFFFFF")
		else
	...
	awesome.connect_signal("signal::bluetooth", function(value)
		if value then
			bluetooth.markup = helpers.colorize_text("", "#FFFFFF")
		else
			bluetooth.markup = helpers.colorize_text("", beautiful.fg_color .. "99")
		end
	end)
	...
	s.wibar_wid:setup({
		{
			...
			},
			{
				...
				},
				{
					...
					margins = { left = dpi(2), top = dpi(8), bottom = dpi(10) },
					widget = wibox.container.margin,
				},
				{
					battery,
					margins = { left = dpi(2), right = dpi(8), top = dpi(6), bottom = dpi(6) },
					widget = wibox.container.margin,
					-- layout = wibox.layout.fixed.horizontal,
				},
				{
